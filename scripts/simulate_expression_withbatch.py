...
if __name__ == '__main__':
    ...
    # Add some batch effects
    GX_batch = np.zeros((opts.ngene, opts.nsample))
    batch1 = np.sort(np.random.choice(opts.nsample, size=int(opts.nsample/2), replace=False))
    GX_batch[:, batch1] = 3.14

    # Total normalized gene expression
    GX_raw =  GX_tmp + GX_trans + GX_batch
